...
appropriately chosen termination criteria\revisedB{; we use a relative
  ...
  tolerance}). \werner{Is there an actual value for this?} The assembly of these nonlinear and linear systems can
be performed in the usual way by looping (in parallel) over cells and
constructing the appropriate contributions upon substituting $w$ and
$\phi$ for each basis function from standard sparse finite element
bases for $V\times Q$. It remains to find a scalable way to solve
(\ref{eq:u J}-\ref{eq:D J}).
...
100.00 &  658.81 &  944.24&  1585.6 & 1.433251 & 1.679234 & 2.406764\\
...
%  10800   &  452.88   &  691.13   &  1157.9  &  1.526078  &  1.675372  &  2.556748\\
  ...
  \end{tabular}
  \caption{Table showing wallclock times for different numbers of cores for the ARK2 IMEX scheme and the Gauss-Legendre IRKs. Similar values are obtained for the Radau IIA IRKs. The results are typical for intra node parallelism, with the usual bandwidth limitations.
    \label{tab:cores}}
\end{table}
...
\revisedA{We observe that the IRKs and DIRKs allow for larger timesteps and
 ...
 \revisedB{We also studied the effect of changing the nonlinear solver
 ...
 \werner{{Is this response to Reviewer 1, then it should be blue!}}
 ...
 \end{tabular}
   \revisedus{
  \caption{Relative errors of Gauss-Legendre 1-3\werner{, Alexander DIRK, } and ARK2 IMEX methods
...
\end{tabular}
   \revisedus{
 \caption{Relative errors of Radau IIA 1-3\werner{, Alexander DIRK, } and ARK2 IMEX methods vs total
...
with $\Delta t=3600$. Plots of the solution at longer times as well
as time series of energy and divergence are shown Figures
\ref{fig:w6_28}-\ref{fig:w5energy}.
\werner{Shouldn't we say something about divergence and energy? What do the figures tell us about robustness?
Do we have to define what Divergence RMS using in the plots below means?
}
...
\begin{figure}
  ...
  \end{center}
    \caption{\label{fig:w6energy}Plots of energy and the norm of the
...
\begin{figure}
  ...
  \end{center}
      \caption{\label{fig:w5energy}Plots of energy and the norm of the
