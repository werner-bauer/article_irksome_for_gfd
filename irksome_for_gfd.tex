...
\todo{\werner{should we mention above that ASM is for all
levels expect coarsest one?

not so clear here! - could we write: We experimented with a direct solver on
the coarses grid or using ASM instead of block ILU, but we
found
}
}
...
\checkit{$\Delta t=100s$} on mesh {refinement} 6 since the method was unstable
for larger timesteps.
\todo{\werner{
- WE HAVE TO UPDATE THIS VALUE: WE HAVE NOW 180s time step maximum, right?


should we mention DIRKs, or do we assuem that they fall under IRKs? I guess it's ok here since we only introduce dirks below!}
}
