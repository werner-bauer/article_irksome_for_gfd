...
\todo{Add information somewhere about using Firedrake and Irksome!}
...
\begin{figure}
  ...
  \caption{\label{fig:patch} A diagram showing the ``vertex
    ...
    patch boundary) \checkit{are equal to zero}.
  }
 \end{figure}
...
\checkit{=\prod_{i=1}^s\left(V \times Q\right)}$, $l=1,\ldots,N_l$,
\checkit{where $W$ is
...
\checkit{
The spatial discretization error is estimated by comparing the solution
between mesh refinement level L6 and L7 after a 1-day simulation using
a very small time step size of $dt =1$. We assume that, for the chosen mesh
levels and time step size, the temporal error is sufficiently small so that
the spatial discretization error dominates for the chosen mesh levels.
We found spatial discretization errors
of $2.542863 \cdot 10^{-05}$ for the depth field $\eta$ and $ 9.777218 \cdot 10^{-06}$ for the
velocity field $u$. This error values are indicted as vertical lines in
...
errors. \werner{Maybe here we could need more explanation!}
As such, we compare these quantities for various parameter
...
was $\Delta t=100s$ on mesh \checkit{refinement} 6 since the method was unstable for larger
timesteps.
...
icosahedron as discussed previously. \checkit{Mesh refinement level $p$}
...
\end{tabular}
 \caption{Relative errors of Radau IIA 1-3 and ARK2 IMEX methods vs total
   ...
   time step sizes \checkit{$\Delta t$} used for the corresponding solution. Solid
   lines indicate values for the elevation field $\eta=D-H$ and dashed lines
   for the velocity fields $ u$.
   \checkit{
   The solid and dashed vertical lines indicate the spatial discretisation
