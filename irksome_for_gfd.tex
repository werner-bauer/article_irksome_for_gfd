...
\werner{$k$?, vectors in bold face?} \colin{I prefer not to, it should
  be inferrable from the context. There are plenty of other objects
  around so it seems odd that vectors should get a special typeface.}
\werner{Ok, but what about $k$???}
...
\begin{align}
    \label{eq:u stages}
    R_{u,i}[w] := \langle w, k_{u,i} \rangle + a\left(
    u^n_i,D^n_i;w\right)
  & = 0,  
...
\werner{Shouldn't be the derivative of the Jacobi with respect to ku and kD while the term itself depends on ku' and kD', so the other way round as you have written it?}
...
\werner{(Define da, dc with 4th entry, i.e derivative of forms a, c
with respect to current stage k'?). Maybe explain only notation, i.e.
what does the new notation mean?}
