...
\subsubsection{Nonlinear solver}
  ...
  approximate solution of the Jacobian system (\ref{eq:u J}-\ref{eq:D J}) followed by the
  ...
  (\ref{eq:u J}-\ref{eq:D J}), which is completed when a termination
  ...
  updated again in the next Newton iteration anyway. Here, relative
  tolerance means a maximum value of the ratio of the equation residual
  divided by the value before the first iteration, below which the
  Jacobian solution procedure is terminated.
  %This reduces the
  ...
  then the Jacobian system iteration
...
method. This is because we used GMRES within the preconditioner (see later), which necessitates a flexible
...
For smoothers on each level above the coarstest,
...
under this refinement scaling without multigrid \revisedus{(as obtained
in \citet{cottershipton2023compatible})}.
However, we found
...
above in the case of $p=1$.
\revisedA{Additionally, we used the same solver approach with the 3 stage diagonally
   implicit Runge-Kutta method (DIRK) of \citet{alexander1977diagonally}.}
We compare these methods with the
...
\begin{figure}
  \centering
 \begin{tabular}{cc}
 \hspace{-0em}\includegraphics[scale=0.3]{Images/vorticity_0.png} &
 \hspace{-0em}\includegraphics[scale=0.3]{Images/elevation_0.png} \\
 \hspace{-0em}\includegraphics[scale=0.3]{Images/vorticity_24.png} &
 \hspace{-0em}\includegraphics[scale=0.3]{Images/elevation_24.png} \\
...
choices for the IRK, \revisedA{DIRK,} and IMEX schemes. As expected, accuracy and
runtime crucially depend on the selected time step sizes; we compute
errors and runtime for IRKs, \revisedA{DIRKs,} and the ARK2 IMEX scheme over a range of
$\Delta t$ values. For the IMEX scheme, the maximum timestep was
\revisedus{$\Delta t=180s$} on mesh {refinement} 6 since the method was unstable
...
\end{table}
 ...
 Newton relative residual tolerance of $1.0\times 10^{-6}$,
 \revisedus{and a multigrid scheme with $p$ levels, where
   $p$ is the mesh refinement used.} For the
 ...
 \revisedA{We also took measurements for the 3 stage DIRK scheme of
...
\revisedA{We observe that the IRKs and DIRKs allow for larger timesteps and
 ...
 \end{tabular}
  ...
  \caption{Relative errors of Gauss-Legendre 1-3, Alexander DIRK, and ARK2 IMEX methods
   ...
   The solid and dashed horizontal lines indicate an estimate of the spatial discretisation error between mesh levels L6 and L7
...
7200 & 1.006 & 1.243 & 1.006 & 1.393 &  1.031 & 1.208 \\
      ...
      \caption{\label{tab:GLntol}Wallclock time ratios for
        ...
        \todo{Order is different from above!!!
        Should I reorder it to match others?}
      }}
  \end{table}
