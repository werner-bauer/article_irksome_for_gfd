...
\begin{align}
    \label{eq:u stages}
    R_{u,i}[w] := \langle w, k_{u,i} \rangle + \Delta ta\left(
    u^n_i,D^n_i;w\right)
  & = 0,  
  \quad \forall w \in V,\mbox{ for }i=1,\ldots,s, \\
  \label{eq:D stages}
  R_{D,i}[\phi] := \langle \phi, k_{D,i} \rangle + \sum_{j=1}^sA_{ij}c\left(u^n_i,D^n_i; \phi\right)
...
In this work, we solve (\ref{eq:u J}-\ref{eq:D J})  a monolithic Krylov solver (GMRES in this
