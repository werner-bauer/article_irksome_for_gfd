...
\end{align}
<<<<<<< HEAD
\werner{index in 7-8 now over $i$ and also $j$, but latter not specified! Maybe use other
indices in 9-10?}
=======
>>>>>>> dc5d003455f17f046bad31c9cb094a140db71bef
...
(\ref{eq:u stages}-\ref{eq:D stages}) for the stages\werner{solving 7-10?} \colin{I think not, because 9-10 are just definitions which should be substituted into 7-8}, the solution
...
full set of basis coefficients for the iterative corrections
$w=\{(k'_{u,i}, k'_{D,i})\}_{i=1}^s$, treated as a single vector. In
our experiments we used FGMRES (the flexible generalised minimum
residual method) for the chosen Krylov method, because we used GMRES
(the generalised minimum residual method) in some inner iterations in the
preconditioner, which we shall describe next.
...
\citep{farrell2021irksome,kirby2024extending}, a Python library
that wraps time discretisations on top of finite element discretisations
implemented using Firedrake \citep{FiredrakeUserManual}, an automated
system for the solution of partial differential equatoins using the
finite element method.
This combination in turn makes significant use
of PETSc \citep{dalcin2011parallel,balay2019petsc}. In particular,
PETSc's and Firedrake's PCPatch implementation
of additive Schwarz methods is used \citep{farrell2021pcpatch}.
...
\begin{align}
    \label{eq:u stages IMEX}
    \langle w, Y_{u,i}-u^n \rangle + \Delta t \sum_{j=1}^{i-1} A_{ij} a_N\left(
    Y_{u,j}, Y_{D,j}; w\right)
    + \Delta t \sum_{j=1}^i \tilde{A}_{ij} a_L(Y_{u,j}, Y_{D,j}; w)
  & = 0,  
  \quad \forall w \in V,\mbox{ for }i=1,\ldots,s, \\
  \label{eq:D stages IMEX}
  ...
  & = 0, 
\end{align}
where $A$ and $\tilde{A}$ are the IMEX Butcher matrices,
before reconstructing the solution from
\begin{align}
  \langle w, u^{n+1} - u^n\rangle + \Delta t\sum_{i=1}^s b_i
  a_N(Y_{u,j}, Y_{D,j}; w)
  + \Delta t \sum_{i=1}^s \tilde{b}_i
  a_L(Y_{u,j}, Y_{D,j}; w)\, \forall w \in V, \\
    \langle \phi, D^{n+1} - D^n\rangle + \Delta t\sum_{i=1}^s b_i
  c_N(Y_{u,j}, Y_{D,j}; \phi)
  + \Delta t \sum_{i=1}^s \tilde{b}_i
    c_L(Y_{u,j}, Y_{D,j}; \phi)\, \forall \phi \in Q, \\
\end{align}
...
for the 2D problems under consideration. Unlike the patch problems
used for the IRK methods which depend on the system state, the
implicit IMEX linear systems can be assembled and factorised once,
which makes their solution very fast.
...
hybridization solver package SLATE \citep{gibson2020slate}.
