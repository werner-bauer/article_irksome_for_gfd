...
\author{Werner Bauer\footnote{Corresponding author:
w.bauer@surrey.ac.uk,
School of Mathematics and Physics,
University of Surrey,
Guildford, GU2~7XH, UK,
ORCID:~\href{https://orcid.org/0000-0002-5040-4287}{https://orcid.org/0000-0002-5040-4287}
}
 ,
Colin J. Cotter\footnote{Imperial College London, UK, ORCID:
\href{https://orcid.org/0000-0001-7962-8324}{https://orcid.org/0000-0001-7962-8324}}
, and
Joshua Hope-Collins\footnote{Imperial College London, UK, ORCID:
\href{https://orcid.org/0000-0003-2051-8627}{https://orcid.org/0000-0003-2051-8627}}
}
...
\newcommand{\werner}[1]{{\color{cyan}WB says: #1}}
\newcommand{\colin}[1]{{\color{blue}CC says: #1}}
\newcommand{\checkit}[1]{{\color{magenta}#1}}
%\renewcommand{\checkit}[1]{{\color{red}#1}}


\newcommand{\revisedA}[1]{{\color{blue}#1}}
\newcommand{\revisedB}[1]{{\color{magenta}#1}}
\newcommand{\revisedus}[1]{{\color{Green}#1}}

% use after pdf with comments had been compiled
% \renewcommand{\revisedA}[1]{#1}
% \renewcommand{\revisedB}[1]{#1}
% \renewcommand{\revisedus}[1]{#1}
...
ARK2 scheme of Giraldo et al (2013), as a benchmark. \st{This provides an
initial look at whether implicit Runge-Kutta methods can be viable for
atmosphere and ocean simulation.}
\werner{Our results illustrate clearly that implicit Runge-Kutta
methods are viable for atmosphere and ocean simulation.}



\smallskip
\noindent
\textbf{Keywords}: Implicit Runga-Kutta methods;
...
atmosphere models (and for large scale numerical solutions of partial
...
implementation, facilitated by the Firedrake
\citep{FiredrakeUserManual}, Irksome \citep{farrell2021irksome} and
PETsc \citep{balay2019petsc} software packages, uses a compatible
finite element spatial discretisation \citep{cotter2023compatible},
but this is not the central focus of the work.
...
\begin{align}
  ...
  + \nabla\left(\frac{|u|^2}{2} + g(D\revisedA{-}b)\right) & = 0, \\
  D_t + \nabla\cdot(uD) & = 0,
\end{align}
...
\todo{\werner{rather than 'hopefully', shouldn
t we better write that we are 'confident that they translate' to other ....?}
} translatable to other discretisation approaches. However, to
efficiently describe our iterative solver approach a precise
description is useful. We select $V$ as the degree $p+1$ BDM
\revisedA{(Brezzi-Douglas-Marini)} finite element space on triangles,
and $Q$ as the degree $p$ discontinuous Lagrange finite element space,
here defined on an icosahedral grid $\mathcal{X}$ formed by
recursively refining an icosahedron and then projecting vertices
radially out to the sphere.  Then we seek $(u,D)\in V\times Q$ such
that
\begin{align}
  \label{eq:ut}
...
$\tilde{D}$ are the values of $D$ and $u$ evaluated on the upwind side
...
values of $\psi$ over both sides of the facet.\footnote{\revisedA{To
  explain the $\jump{\cdot}$ operator further, we consider labelling
  the two sides of each edge with $+$ and $-$, so that $\psi^-$
  indicates the value of $\psi$ on the $-$ side, etc., thus
  $\jump{\psi}=\psi^++\psi^-$. Note that when $n$ is present,
  $n^+=-n^-$, and hence e.g. $\jump{\phi n \cdot u}=\phi^+n^+\cdot u^+
  + \phi^+n^-\cdot u^- = n^+(\phi^+u^+ - \phi^-u^-)$, so this gives
  rise to a jump. This particular choice of notation is used because
  of the appearance of $n^\perp$, which does not match standard
  patterns in mixed formulations of elliptic problems or discontinuous
  Galerkin discretisations for scalar transport.}} \revisedB{Here, the
  semicolons in the forms $a(u,D;w)$ and $c(u,D;\phi)$ indicate the
  linearity of the operators in the test functions $w$ and $\phi$,
  respectively. The forms can be nonlinear in the solution variables;
  this is a standard formulation for finite element methods applied
...
\subsection{Implicit Runge-Kutta time discretisation
  \revisedA{and the resulting implicit systems}}
...
particular chosen Runge-Kutta method. \revisedB{Here, the stages are
  values of the time derivatives evaluated at different points in time
  relative to the current timestep; this is referred to as the ``stage
  derivatives'' formulation. There is an alternative ``stage values''
  formulation of Runge-Kutta methods where one solves for the value
  of the solution at different points instead, but we do not consider
  it here.}

Our spatially discrete rotating
...
\revisedB{Here, we describe the stage derivatives formulation of IRKs, where
  time derivatives are solved for at each step. There is an alternative
  stage values formulation, which we do not consider here.}
  \todo{\werner{It seems that this short paragraph is a repetition from above and can be removed!}

  -  are eqn 10-11 correct? where is the n+1? I think it is correct, I just want to be sure!
  }
...
oscillations.  \revisedB{See the two volume textbook \citet{hairer1993solving,wanner1996solving} for a comprehensive
derivation along with their Butcher
tableau (Chapter II.7), and stability and convergence analysis (Chapters IV.3-IV.5).}  Note here that our system has no explicit time dependence
which would otherwise need to be incorporated into (\ref{eq:u
  stages}-\ref{eq:D stages}) in the usual way.

\revisedA{In this article we will discuss Newton-type methods for solving
 (\ref{eq:u stages}-\ref{eq:D
  stages}).} Given an initial guess $k\in W$ for
...
\end{align}
In this work, we
take the convention that the upwind switches have derivative
zero\footnote{This treatment means that we {are already considering a
quasi-Newton method here}; using a semi-smooth Newton method might be
...
appropriately chosen termination criteria\revisedB{; we use a relative
  tolerance test requiring that the ratio of the norm of the residual
  to the residual coming from the initial guess is below a selected
  tolerance, given later}). The assembly of these nonlinear and linear
systems can be performed in the usual way by looping (in parallel)
over cells and constructing the appropriate contributions upon
substituting $w$ and $\phi$ for each basis function from standard
sparse finite element bases for $V\times Q$. It remains to find a
scalable way to solve (\ref{eq:u J}-\ref{eq:D J}).

\subsection{\revisedA{Scalable iterative methods}}

\revisedA{In this section, we describe our iterative approach to
  solving (\ref{eq:u stages}-\ref{eq:D stages}). This involves
  the composition of several stages, which we illustrate in Figure
  \ref{fig:solver_diag}.

\subsubsection{Nonlinear solver}
  
  At the outer level, (\ref{eq:u stages}-\ref{eq:D stages}) is solved
  using a quasi-Newton method, each iteration consisting of the
  approximate solution of (\ref{eq:u J}-\ref{eq:D J}) followed by the
  update \eqref{eq:newton update}. These iterations are repeated until
  a relative tolerance condition is achieved, requiring that the norm
  of the residual in (\ref{eq:u stages}-\ref{eq:D stages}) is below
  some chosen tolerance multiplied by the residual norm before the
  first iteration. The choice of tolerance for our experiments is
  discussed in Section \ref{sec:results}.

  In the quasi-Newton method, an iterative method is used to solve
  (\ref{eq:u J}-\ref{eq:D J}), which is completed when 
  \todo{\werner{sentence not complete!!!}
  }}
  
  Our quasi-Newton method uses the Eisenstat-Walker (version 2)
  inexact Newton approach \citep{eisenstat1996choosing} which
  adaptively controls the {relative tolerance for iterative
    solution of Jacobian system (\ref{eq:u J}-\ref{eq:D J}) at each
    Newton step, since it is not useful to solve the Jacobian
  system accurately when the nonlinear solution is only going to be
  updated again in the next Newton iteration anyway.
  %This reduces the
  %overall number of Krylov iterations per timestep, and hence reduces
  %the overall number of multigrid cycles which dominate the cost of
  %the method.
  To avoid over solving the Jacobian system during the final Newton
  iteration, we also set an absolute tolerance equal to $0.1\times
  \tau$ multiplied by the norm of the starting nonlinear residual,
  \todo{\werner{shall we also write absolute here?}}
  where $\tau$ is the chosen nonlinear tolerance. If the true
  (\emph{i.e.} unpreconditioned) residual falls below this tolerance,
  then the Krylov method iteration
  \werner{'described below'???}
  is terminated.  This works because
  near convergence of the nonlinear solver, the true residual for the
  Jacobian system is very close to the true residual for the nonlinear
  system. Hence, if the true residual for the Jacobian system is 10
  times less than the tolerance set for the nonlinear residual, then
  we expect that the nonlinear tolerance termination criteria to be
  satisfied on the start of the next iteration.

  Finally, we also ``lagged'' the Jacobian, so that it is only updated
  every 20 Newton iterations (including between Newton solves). This
  reduces the cost of rebuilding factorisations in the patch
  smoothers, at the expense of potentially increasing the number of
  required Newton iterations.

\subsubsection{Krylov method for Jacobian system}

\werner{Can we start writing here: For the inner loop, we use
a Krylov method to solve the above described Jacobian system ???}
...
presents challenges for the solver. In this work, we solve (\ref{eq:u
 ...
 \todo{ \werner{in the inner loop}}
a preconditioned Krylov solver to the full set of basis coefficients
for the iterative corrections $k'$, treated as a single vector. In our
experiments we used FGMRES (the flexible generalised minimum residual
method \citep{saad1993flexible}) for the chosen Krylov
method. This is because we used GMRES in some inner
  iterations in the preconditioner (see later), which necessitates a flexible
  Krylov method outside.}

\todo{\werner{Sorry, I don't fully understand where the outer and where
the inner loop is!}}

\begin{figure}
  \center
  \includegraphics[width=5cm]{Images/patch}
  \caption{\label{fig:patch} A diagram showing the ``vertex star''
    patch associated with the vertex at the middle of the
    ...
    patch boundary) are excluded from the patch. \revisedus{This
      diagram shows the degrees of freedom for a single stage, so for
      an $s$-stage scheme the number of degrees of freedom in the
      implicit coupled system is $s$ times bigger.  }}
 \end{figure}

\subsubsection{Preconditioner for the Krylov method}
...
midpoint of each edge. \revisedus{Hence, the meshes can be described as
  being topologically nested but not geometrically nested.}
The new vertices are then moved to the surface
...
For smoothers on each level,
\todo{\werner{how many levels do we use? 2 GM levels?}}
we used an additive Schwarz method
(ASM) that
is built using overlapping subspaces $W_l=V_l\times Q_l \subset W
{=\prod_{i=1}^s\left(V \times Q\right)}$, $l=1,\ldots,N_l$,
{where $W$ is
...
\todo{\werner{are these related to the linearlized RSWE?}}
For the additive
...
\begin{tikzpicture}[node distance = 3cm]
  % Place nodes
  \node [block] (NL) {Nonlinear system};
    \node [block, below of=NL] (Newton) {Eisenstat Walker Quasi-Newton};
    \node [block, right of=Newton, node distance=6.5cm] (FGMRES) {FGMRES};
    \node [block, right of=FGMRES, node distance=3.5cm] (MG) {Multigrid};
    \node [block, right of=MG, node distance=6.8cm] (Coarse) {GMRES 3 its};
    \node [block, right of=Coarse, node distance=3.5cm] (CPC) {Block ILU};
    \node [block, below of=Coarse] (Levels) {GMRES 3 its};
    \node [block, right of=Levels, node distance=3.5cm] (LPC) {Patch ASM};
    % Draw edges
    \path [line] (NL) -- node[right]{Nonlinear solver}(Newton);
    \path [line] (Newton) -- node[below]{linear Jacobian solver}(FGMRES);
    \path [line] (MG) -- node [below]{Coarse grid smoother} (Coarse);
    \path [line] (Coarse) -- node [below]{PC} (CPC);
    \path [line] (Levels) -- node [below]{PC} (LPC);
    \path [line] (MG) |-node [below right]{Levels smoother}  (Levels);
    \path [line] (FGMRES) -- node [below]{PC} (MG);
\end{tikzpicture}
} \revisedus{\caption{\label{fig:solver_diag}Diagram showing the
    composition of solver components for our approach. The boxes
    indicate solvers which have components indicated by the
    arrows. Here PC indicates a preconditioner.}}
\end{figure}
...
depend on the system state, our smoothers consist of \revisedus{3}
iterations of GMRES \revisedus{(the generalised minimum residual
  method \citep{saad1986gmres})} preconditioned by the additive
Schwarz method above. This has the effect of selecting the scaling
parameter adaptively.  This necessitates the use of a flexible Krylov
method (FGMRES, in our case) for the ``outer'' monolithic solver,
since the use of GMRES here means that the smoother is residual
dependent and hence is not a stationary iterative method (which is a
requirement for standard Krylov methods such as GMRES).

In our multigrid setup, we \revisedus{used 3 iterations of GMRES
  preconditioned by block ILU (incomplete LU factorisation)} for the
correction on the coarsest grid. This avoids having to use a parallel
direct solve. We experimented with a direct solve on the coarse grid
but found that it did not alter the overall convergence of the solver
strategy\revisedus{, but it did increase wallclock time.}

\todo{\werner{why not ASM on coarsest to much stuff easier?}}
...
under this refinement scaling without multigrid (as obtained
in \citet{cotter2023compatible}).
However, we found
that using the multigrid produced faster wallclock times than
simply applying the patch smoother on the finest grid.

\subsubsection{Solver software implementation}

\revisedA{ In this work we implemented this approach using Irksome
  ...
  This combination in turn makes significant use of PETSc
  \citep{dalcin2011parallel,balay2019petsc}. In particular, PETSc's
  and Firedrake's PCPatch implementation of additive Schwarz methods
  is used \citep{farrell2021pcpatch}, via the Python preconditioner
  interface of \citet{kirby2018solver}.  Amongst the benefits of this
  approach to implementation, Firedrake uses automated differentiation
  provided by the Unified Form Language (UFL) \citep{alnaes2012ufl} to
  obtain the formulae for (\ref{eq:u J}-\ref{eq:D J}) from which code
  assembling the matrix-vector action and the patch linear systems is
  automatically generated. In our implementation, we use $\ell_2$
  Euclidean norms of vectors of finite element basis coefficients to
  measure residual norms for termination criteria.  }
...
$(Y_{u,i},Y_{D,i})$ at stage $i$. Further, \revisedus{because the
  linear component of the splitting contains only the inertia-gravity
  wave system,} these problems can be reduced to a sparse ``modified
Helmholtz'' type problem for a single variable using the hybridisation
technique for mixed finite elements
...
be refactorised \revisedus{each time the nonlinear system Jacobian is
  recomputed}. The principle disadvantage with the IMEX scheme is that
it is stable conditional on the advective Courant number, which means
that it is limited to smaller timesteps.  It is this property that we
shall contrast with the IRKs.
...
frequently used, very efficient IMEX method ARK2. \revisedA{The goal
  is not to compete with the IMEX scheme, but rather to provide a
  point of reference using an approach that is familiar to the community,
  to give an idea of the viability of IRK schemes.}
...
\ref{tc6_fields}. We compute timestepping errors at relatively short time of 1 day.
...
\begin{figure}
 ...
 \end{tabular}\vspace{-10pt}
  \caption{Top row: initial vorticity (left column) and depth (right
    ...
    cells, 61440 height degrees of freedom and \revisedus{153600} velocity degrees
    of freedom. \revisedus{The reduced system in the IMEX scheme has 92160
    degrees of freedom.}}
 \label{tc6_fields}
\end{figure}
...
As a benchmark to contextualise the timestepping errors, we
provide an indication of the spatial discretization error. This is
done by comparing the solution between mesh refinement levels L6 and
L7 after a 1 day simulation using a very small time step size of $\Delta t
=1s$ so that for the chosen mesh levels and time step size, the
temporal error is negligible compared to the spatial discretization
error.  We found spatial discretization errors of $2.542863 \cdot
10^{-05}$ for the depth field $D$ and $ 9.777218 \cdot 10^{-06}$
for the velocity field $u$. These error values are indicted as \revisedus{horizontal
lines} in Figures \ref{err_vs_runtime_gl_imex} and
\ref{err_vs_runtime_rd_imex}. The point is that reducing the timestepping
error much below these values will not lead to an overall reduction in
the combined spatial and temporal error, so there is no benefit in
reducing the time stepping error further.

In our experiments, essential quantities of interest are the total runtime of a
...
errors and runtime for IRKs and the ARK2 IMEX scheme over a range of
$\Delta t$ values. For the IMEX scheme, the maximum timestep was
$\Delta t=100s$ on mesh {refinement} 6 since the method was unstable
for larger timesteps.
\todo{\werner{should we mention DIRKs, or do we assuem that they fall under IRKs?}
}
...
icosahedron as discussed previously. {Mesh refinement level $p$}
 % p same symbol as for FE space order
 has $20\times 4^p$ cells, so there are $80\times 4^p$ layer depth degrees
...
\begin{table}[h]\centering
  \begin{tabular}{ccccccc}
   ...
   180.00 &  437.0420 &  702.3965 &  1134.2820 & 1.60716 & 1.614874 & 2.595362\\
   18.75 &   4094.4577 & 6703.4870 & 10742.4779 & 1.63721 & 1.602521 & 2.623663 \\
    \multicolumn{7}{c}{Gauss-Legendre 1 stage} \\
  7200 &  91.961 & 134.849 & 224.160 & 1.466372 & 1.662304 & 2.437555\\
  1200 & 186.878 & 279.982 & 472.217 & 1.498207 & 1.686598 & 2.526873\\
   300 & 378.757 & 573.468 & 958.353 & 1.514079 & 1.671153 & 2.530258\\
  \multicolumn{7}{c}{Gauss-Legendre 2 stage} \\
  7200 & 181.891 &  270.150 &  456.909 & 1.485230 & 1.691316 & 2.511993\\
  1200 & 482.581 &  738.246 & 1247.484 & 1.529787 & 1.689794 & 2.585025\\
   300 & 979.752 & 1514.306 & 2567.877 & 1.545601 & 1.695745 & 2.620946\\
    \multicolumn{7}{c}{Gauss-Legendre 3 stage} \\
  7200 &  367.896 &  485.772 &  823.797 & 1.320406 & 1.695851 & 2.239212\\
  1200 & 1168.521 & 1790.578 & 3068.695 & 1.532346 & 1.713801 & 2.626136\\
   300 & 1957.424 & 3070.712 & 5300.002 & 1.568752 & 1.725985 & 2.707641\\
  \end{tabular}
  \caption{\revisedus{
  Table showing wallclock times for different numbers of cores for the ARK2 IMEX scheme and the Gauss-Legendre IRKs. Similar values are obtained for the Radau IIA IRKs. The results are typical for intra node parallelism, with the usual bandwidth limitations.
    \label{tab:cores}
    }
    }
\end{table}
 ...
 For the IRKs we used the solver strategy discussed above with a
 Newton relative residual tolerance of $1.0\times 10^{-6}$. For the
 IMEX scheme we used the SUPERLU Dist parallel direct solver package
 ...
 the mass matrix) \revisedus{to a tolerance of $1.0\times 10^{-10}$,
   and the mass matrix system (\ref{eq:mass D}) was solved using a
   direct solve since it is block diagonal. These systems have minor
   effect} on the solve times compared to the two implicit solves per
 timestep.

 \revisedA{We also took measurements for the 3 stage diagonally
   implicit Runge-Kutta method (DIRK) of
   \citet{alexander1977diagonally}, a third order method.  The
   diagonal structure means that the stages can be solved one by one
   instead of simultaneously, although a separate nonlinear system
   is still required to be solved at each stage.
   The system for each stage is solved using
   the same iterative solver framework as for the IRKs.}
 ...
 Radau IIA IRKs, and the Alexander DIRK; results for the ARK2 IMEX
 scheme are shown in both figures as a reference. Mesh refinement 6
 was used. We considered 1, 2 and 3 stage versions of the IRKs, as we
 were limited by memory on the workstation from examining more stages
 at this resolution. The Gauss-Legendre $s$-stage schemes are order
 $2s$ with stage order $s$ (the relevant order for stiff problems at
 large $\Delta t$), whilst the Radau IIA schemes are order $2s-1$ with
 stage order $s$.


\revisedA{We observe that the IRKs and DIRKs allow for larger timesteps and
 hence quicker time to solution. The DIRK solution provides equivalent
 error at slower time than the IRKs: the collocation schemes provide
 higher accuracy for the same timestep, which is best seen by
 comparing the 2 stage Radau IIA scheme with the DIRK (they are both
 third order methods). We also see that the Gauss-Legendre schemes
 provide quicker time-to-solution for the same error than the Radau
 IIA schemes.  This is because they have one less stage per order. The
 plots also show that the IRKs provide a viable option since they are
 certainly not slower than the IMEX scheme in general.}
 
 The loss of $\Delta t$ robustness for larger $\Delta t$ is demonstrated
 in Tables \ref{tab_niter_vs_dt G} and \ref{tab1_niter_vs_dt R}. We observe
 $\Delta t$ robustness for smaller $\Delta t$, with more sustained $\Delta t$
 robustness for the higher order schemes.

 \revisedA{We also studied the effect of changing the nonlinear solver
   tolerance from $10^{-6}$ to $10^{-5}$ and $10^{-7}$. There was
   insignificant changes to the relative solution error values
   (overall no bigger than $5\times 10^{-8}$), but there was a change in
   wallclock times, due to changes in the number of Newton
   iterations. When the tolerance was decreased, more Newton iterations
   were required in general, increasing the wallclock time (and similarly
   for increasing the tolerance.)
   These changes are presented in Table \ref{tab:GLntol}.}

 \begin{figure}\centering
 \begin{tabular}{c}
  \hspace{-1.3cm}\includegraphics[scale=0.62]{Images/gl_imex.pdf}
 \end{tabular}
   \revisedus{
  \caption{Relative errors of Gauss-Legendre 1-3, Alexander DIRK, and ARK2 IMEX methods
   ...
   The solid and dashed vertical lines indicate \revisedus{an estimate of }the spatial discretisation
   error between mesh levels L6 and L7
   of $\eta$ and $u$, respectively.
   }
...
\end{figure}
 ...
 \begin{figure}\centering
\begin{tabular}{c}
 \hspace{-1.3cm}\includegraphics[scale=0.62]{Images/rd_imex.pdf}
\end{tabular}
   \revisedus{
 \caption{Relative errors of Radau IIA 1-3, Alexander DIRK, and ARK2 IMEX methods vs total
   wallclock time. The numbers on the curves indicate the
   time step sizes {$\Delta t$} used for the corresponding solution. Solid
   lines indicate values for the elevation field $\eta=D-H$ and dashed lines
   for the velocity fields $ u$.
   The solid and dashed vertical lines indicate the spatial discretisation
   error between mesh levels L6 and L7
   of $\eta$ and $u$, respectively.
   }
...
\begin{table}
  ...
  \begin{tabular}{ccccccc}
   ...
   &  its/steps  &  its 
    \\ \hline
7200 & 11.83 & 142.00 & 8.92 & 107.00 & 7.92 & 95.00 \\
3600 & 5.96 & 143.00 & 4.38 & 105.00 & 4.17 & 100.00 \\
2400 & 4.03 & 145.00 & 4.00 & 144.00 & 4.00 & 144.00 \\
1200 & 4.00 & 288.00 & 4.00 & 288.00 & 4.96 & 357.00 \\
600 & 3.00 & 432.00 & 3.01 & 433.00 & 3.01 & 433.00 \\
300 & 2.00 & 577.00 & 2.00 & 576.00 & 2.05 & 589.00 \\
\end{tabular}
\caption{Krylov iterations per step (its/steps) and total Krylov iteration count (its) vs $\Delta t$ for the Gauss-Legendre  1, 2, 3 methods.}
...
\begin{table}[h]
  \revisedus{\centering
   ...
   &  its/steps  &  its 
    \\ \hline
7200 & 23.08 & 277.00 & 12.83 & 154.00 & 9.83 & 118.00 \\
3600 & 9.71 & 233.00 & 6.21 & 149.00 & 5.04 & 121.00 \\
2400 & 6.94 & 250.00 & 4.06 & 146.00 & 4.11 & 148.00 \\
1200 & 4.03 & 290.00 & 4.00 & 288.00 & 4.12 & 297.00 \\
600 & 4.00 & 576.00 & 4.00 & 576.00 & 4.00 & 576.00 \\
300 & 3.00 & 864.00 & 3.00 & 864.00 & 2.25 & 647.00 \\
\end{tabular}
\caption{Krylov iterations per step (its/steps) and total Krylov iteration count (its) vs $\Delta t$ for the RADAU IIA 1, 2, 3 methods.}
...
\begin{table}[h]
 ...
 & \multicolumn{2}{c}{GL1} & \multicolumn{2}{c}{GL2} & \multicolumn{2}{c}{GL3}\\
        $\Delta t$ &
        $T_{10^{-6}}/T_{10^{-5}}$ &
        $T_{10^{-7}}/T_{10^{-6}}$ &
                  $T_{10^{-6}}/T_{10^{-5}}$ &
        ...
        $T_{10^{-7}}/T_{10^{-6}}$ \\
\hline
300 & 1.221 & 1.078 & 0.957 & 1.230 &   1.217 & 1.093 \\
600 & 1.153 & 1.017 & 1.230 & 1.052 &   1.101 & 1.067 \\
1200 & 1.292 & 1.147 & 1.091 & 1.316 &  1.375 & 1.160 \\
2400 & 1.252 & 1.183 & 1.023 & 1.188 &  1.266 & 1.148 \\
3600 & 1.465 & 1.159 & 1.014 & 1.361 &  1.007 & 1.009 \\
7200 & 1.006 & 1.243 & 1.006 & 1.393 &  1.031 & 1.208 \\

      \end{tabular}
      \caption{\label{tab:GLntol}Wallclock time ratios for
        Gauss-Legendre IRKs (stages 1,2,3) for various relative
        tolerances. Here, $T_{\tau}$ indicates the time to solution
        when nonlinear relative tolerance $\tau$ is used.
      }}
  \end{table}

\revisedB{
Finally, to demonstrate robustness of the numerical schemes we ran two
testcases for longer time periods, the mountain test case ``5'' in
addition to the Rossby-Haurwitz wave test case ``6'', both of
\citet{WILLIAMSON1992211}. We used the 2 stage Gauss-Legendre scheme for this,
with $\Delta t=3600$. Plots of the solution at longer times as well as
time series of energy and root mean square (RMS) of the divergence are shown in
Figures \ref{fig:w6_28}-\ref{fig:w5energy}. We observe that they are
both under control, indicating that the scheme is stable and not generating
spurious inertia-gravity waves.
}

\begin{figure}
  \begin{center}
    \includegraphics[scale=0.4]{Images/w6_elevation_28days}
  \end{center}
  \caption{\label{fig:w6_28}The elevation after 28 days in the Rossby-Haurwitz
    test case using $\Delta t=3600$ and the 2-stage Gauss-Legendre scheme.}
\end{figure}

\begin{figure}
  \begin{center}
    \includegraphics[scale=0.6]{Images/Williamson 6_energy.png}
    \includegraphics[scale=0.6]{Images/Williamson 6_divergence.png}
  \end{center}
    \caption{\revisedB{\label{fig:w6energy}Plots of energy and the RMS of the
      divergence versus timestep over 50 days in the Rossby-Haurwitz
      test case using $\Delta t=3600$ and the 2-stage Gauss-Legendre scheme.}}
\end{figure}

\begin{figure}
  \begin{center}
    \includegraphics[scale=0.4]{Images/w5_elevation_15days.png}
    \includegraphics[scale=0.4]{Images/w5_vorticity_50days.png}
  \end{center}
  \caption{\revisedB{\label{fig:w5_long}Top: The elevation after 15 days in the
    mountain test case using $\Delta t=3600$ and the 2-stage
    Gauss-Legendre scheme. Bottom: The vorticity for the same test
    after 50 days.}}
...
\begin{figure}
  \begin{center}
    \includegraphics[scale=0.6]{Images/Williamson 5_energy.png}
    \includegraphics[scale=0.6]{Images/Williamson 5_divergence.png}
  \end{center}
      \caption{\revisedB{\label{fig:w5energy}Plots of energy and RMS the
      divergence versus timestep over 50 days in the mountain
      test case using $\Delta t=3600$ and the 2-stage Gauss-Legendre scheme.}}
\end{figure}
...
An iterative solver strategy \revisedus{using multigrid and additive
  Schwarz smoothers applied to the monolithic coupled system for all
  variables at all stages} was introduced for implicit Runge-Kutta
  methods applied to the rotating shallow water equations on the
  sphere, in conjunction with a compatible finite element
  discretisation. \revisedus{These patch systems are dense, and become reasonably
  large as $s$ increases ($144\times 144$ for $s=3$), which might be
  useful for modern computing architectures where floating point
  operations can be less important than data movement, so these
  methods are worth looking into now (although we do not explore this
  aspect here).}

  We compared wallclock times and timestepping error
  for Gauss-Legendre and Radau IIA implicit collocation schemes with
  1, 2 and 3 stages with the ARK2 IMEX scheme. Our results demonstrate
  that these schemes \revisedus{achieve a comparable time to solution
    to the IMEX scheme for comparable accuracy}, and can provide faster
  wallclock times if less accuracy is needed. Additionally, our solver
  approach could definitely benefit from further performance tuning,
  since the Firedrake multigrid \revisedus{implementation} and patch
  frameworks are probably not optimal, whilst the direct solver used
  for the implicit solve in the IMEX scheme probably is\revisedus{,
    for problems of this size}. Our results also demonstrate the
  capability of the combination of Irksome, Firedrake and PETSc to
  apply sophisticated solver strategies for IRKs.
...
already required.
\revisedus{Hence, it is possible that IRKs will continue to be
  a competitive and viable option for} this larger scale setting.
We will investigate this in future work.

\section*{Acknowledgements}
CC \revisedus{and JHC} acknowledge the support of the Met Office via the Strategic Priorities Fund ExCALIBUR grant ``EX20-8 Exposing Parallelism: Parallel-in-Time''.

\section*{Code availability}
\revisedB{The version of the source code for this paper is archived at
\citet{colin_j_cotter_2026_20817145}. The version of Firedrake, PETSc
and other components is archived at
\citet{firedrake_zenodo_2026_20814010}.}
