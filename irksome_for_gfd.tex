...
method\revisedA{. This is because we used GMRES in some inner
...
\begin{figure}
  ...
  \caption{\label{fig:patch} A diagram showing the ``vertex
    ...
    patch boundary) are excluded from the patch.
  }
 \end{figure}
...
\begin{tikzpicture}[node distance = 3cm]
  ...
  \node [block] (NL) {Nonlinear system};
    ...
    \path [line] (Newton) -- node[below]{linear Jacobi solver}(FGMRES);
...
\end{tikzpicture}
    ...
    indicate solvers which have components indicated by the arrows.}}
\end{figure}
...
In final optimisations, we used the Eisenstat-Walker (version 2)
inexact Newton approach \citep{eisenstat1996choosing} which adaptively
controls the \revisedus{relative tolerance for the Krylov method
  solving Jacobian system (\ref{eq:u J}-\ref{eq:D J}) during the
  Newton algorithm, since it is not useful to solve the Jacobian
  system accurately when the nonlinear solution is only going to be
  updated again in the next Newton iteration anyway. This reduces the
  ...
  the method. To avoid excessive Krylov iterations in the final Newton
  iteration, we also set an absolute tolerance for the Krylov method
  equal to $0.1\times \tau$ multiplied by the norm of the starting
  nonlinear residual, where $\tau$ is the chosen nonlinear tolerance.
  If this number is too large, the nonlinear tolerance would not be
  reached, triggering another nonlinear iteration (losing any gains).
  We also ``lagged'' the Jacobian, so that it is only updated every
  20 Krylov iterations (including between Newton solves). This reduces
  the cost of rebuilding factorisations in the patch smoothers, at
  the expense of potentially increasing the number of required Newton
  iterations.
}

\revisedus{A diagram showing the composition of these various solvers
is given in Figure \ref{fig:solver_diag}.
...
under this refinement scaling without multigrid.
However, we found
...
\begin{figure}
 ...
 \end{tabular}\vspace{-10pt}
  \caption{Top row: initial vorticity (left column) and depth (right
    ...
    cells, 61440 height degrees of freedom and 92160 velocity
    degrees of freedom.}
 \label{tc6_fields}
\end{figure}
...
for the velocity field $u$. These error values are indicted as vertical
...
\end{table}
 ...
 the mass matrix) \revisedus{to a tolerance of $1.0\times 10^{-10}$,
   ...
   affect} on the solve times compared to the two implicit solves per
 timestep.
 ...
 \revisedA{We also took measurements for the 3 stage diagonally
   ...
   instead of simultaneously. The system for each stage is solved using
...
\revisedA{We observe that the IRKs and DIRKs allow for larger timesteps and
 ...
 \revisedA{We also studied the effect of changing the nonlinear solver
   ...
   iterations. These changes are presented in Table \ref{tab:GLntol}.}
...
\end{tabular}
\caption{Iteration per step (its/steps) and total iteration count (its) vs $\Delta t$ for the RADAU IIA 1, 2, 3 methods.}
...
\citet{WILLIAMSON1992211}. We used the Gauss-Legendre scheme for this,
...
\begin{figure}
  ...
  \end{center}
    \caption{\label{fig:w6energy}Plots of energy and the RMS of the
      divergence versus timestep over 28 days in the Rossby-Haurwitz
      test case using $\Delta t=3600$.}
\end{figure}
...
\begin{figure}
  ...
  \end{center}
  \caption{\label{fig:w5_long}Top: The elevation after 15 days
    in the mountain test case using $\Delta t=3600$. Bottom:
  The vorticity for the same test after 50 days.}
...
\begin{figure}
  ...
  \end{center}
      \caption{\label{fig:w5energy}Plots of energy and RMS the
...
CC acknowledges the support of the Met Office via the Strategic Priorities Fund ExCALIBUR grant ``EX20-8 Exposing Parallelism: Parallel-in-Time''.

\section*{Code availability}
The version of the source code for this paper is archived at
