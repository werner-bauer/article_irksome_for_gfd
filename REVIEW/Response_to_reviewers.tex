...
\author{Werner Bauer, Joshua Hope-Collins, and Colin J. Cotter}
...
tolerance in the IMEX mass matrix solve being too loose (it was not
actually solving to the specified tolerance due to a misunderstanding
about defaults in the solver interface that was used), so it was
effectively solving a (slightly) different ODE. Third, we realised that we were
using the value of $H$ for the solid rotation component of the
Rossby-Haurwitz problem, not the global mean of the initial layer
depth.  With this change, the maximum timestep allowed by stability
increased, but the errors increased as well. We have changed all of
this, and it now leads to slightly slower IMEX results (the solve is
not dominated by the mass solve but it does add a little time). We
apologise to the Editor and Reviewer for these inaccuracies in the
first manuscript.

Since he was significantly involved in the improvements to the iterative
method, and discovering the issue with the IMEX results, we have added
Joshua Hope-Collins to the manuscript. He has also contributed to the
preparation of the revised document and this response.
...
changes. Our own additional changes are marked in green.
...
\item Page 3: the notation $[[ \cdot ]]$ for the average is uncommon
  and reminds more of jump rather than an average\\
  \emph{In fact, this
...
\item Section 2.2 is somewhat chaotic, there are many different
  ...
  used in the end.\\
  \emph{This is a very good suggestion, we have added a diagram. We have
  also reorganised the text a bit, separating the description of the
  implicit systems with the algorithm for their solution, and rearranged
  the order to match the progression of the diagram.}
